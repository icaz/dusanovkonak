<?php
    ...
    if (isset($_POST['nick']) && $_POST['nick'] != "")
    {
        ...
        $resultn = $mysqli->query("SELECT * FROM users WHERE nick='$nick'");
        if ($resultn->num_rows == 0)
        {
          $nick_validate=' is-valid';
          $nick_value=' value="'.$nick.'"';
          
          $err_nick='<div class="valid-feedback">
          Ime --'.$nick.'-- je slobodno.
          </div>';
        }
        elseif ($resultn->num_rows > 0)
        {
          $nick_validate=' is-invalid';
          while ($resultn->num_rows != 0)
          {
            echo $nick2=$nick.$f;
            $f=$f+1;
            $resultn = $mysqli->query("SELECT * FROM users WHERE nick='$nick2'");
          }
          $nick_value='value="'.$nick2.'"';
          $err_nick='<div class="invalid-feedback">'.$nick.'
          je zauzeto, probajte sa: <strong class=" is-valid">'.$nick2.'</strong></div>';
        ...
        if (isset($_POST['email']) && $_POST['email'] != "")
        {
            ...
            $resulte = $mysqli->query("SELECT * FROM users WHERE email='$email'");
    ...
    elseif ($pass == $repass)
    {
      ...
      $sql = "INSERT INTO users (nick, email, password, hash, reg_ip)" 
            ."VALUES ('$nick','$email','$pass', '$hash', '$reg_ip')";
    ...
    <div class="card-body register-card-body">
      ...
      <form action="register.php" method="post">
        <div class="input-group mb-3">
          <input name="nick" type="text"  class="form-control <?php echo $nick_validate; ?>" placeholder="Kako se zoveš?" <?php echo $nick_value; ?> required>
