...
if (isset($_POST['btn']) && $_POST['btn']=="1")
{
    if (isset($_POST['nick']) && $_POST['nick'] != "")
    {
        ...
        $result = $mysqli->query("SELECT * FROM users WHERE name='$nick'");
        if ($result->num_rows > 0)
            {
              $_SESSION['nick']='Već postoji korisnik sa imenom <strong> '.$nick.'.</strong><br> To možete kasnije promeniti u profilu...';
            }
        if (strlen($nick) > 3)
        {
        ...
        else
        {
          ...
          $nick_value='value="'.$nick.'"';
          $err_nick='<div class="invalid-feedback">Ime mora biti više od 3 karaktera</div>';
        }
    }
